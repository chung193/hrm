...
import { getAll, storage, bulkDestroy, mediaExport, update } from './MediaServices';
import { DataGrid } from '@mui/x-data-grid';
import Toolbar from '@components/ToolBar';
import getColumns from './MediaColumns.jsx';
import { getBreadcrumbs } from './MediaBreadcrumb';
import MediaAdd from './MediaAddModal.jsx';
import { saveAs } from 'file-saver';
import MetaData from '@components/MetaData';
import { loadListViewOptions, saveListViewOptions } from '@utils/listViewOptions';

const STORAGE_KEY = 'list-view-options:media';
...
const Media = () => {
    ...
    const addMedia = (data) => {
        ...
        storage(data)
            .then(() => {
                showNotification('Thêm media thành công', 'success');
                loadData();
                closeModal();
            })
    ...
    const handleDelete = () => {
        showConfirm(
            t('pages.media.deleteConfirmTitle'),
            t('pages.media.deleteConfirmMessage', { count: selectedRows.size }),
            doDelete,
            closeModal
        );
    };
    ...
    const handleExportExcel = async () => {
        try {
            ...
            const response = await mediaExport({ keyword });

            const blob = new Blob([response.data], {
                type: response.headers['content-type'],
            });

            saveAs(blob, `media_${new Date().toISOString().split('T')[0]}.xlsx`);
    ...
    return (
        ...
        >
            ...
            <Toolbar
                ...
                handleAdd={() =>
                    openModal(
                        t('pages.media.addMedia'),
                        <MediaAdd onSubmit={addMedia} onClose={closeModal} />
                    )
                }
