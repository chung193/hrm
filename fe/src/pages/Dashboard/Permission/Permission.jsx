...
import { getAll, storage, bulkDestroy, permissionExport, update } from './PermissionServices.js';
import { DataGrid } from '@mui/x-data-grid';
import Toolbar from '@components/ToolBar';
import getColumns from './PermissionColumns.jsx';
import { getBreadcrumbs } from './PermissionBreadcrumb.js';
import RoleAdd from './PermissionAddModal.jsx';
import { saveAs } from 'file-saver';
import MetaData from '@components/MetaData';
import { loadListViewOptions, saveListViewOptions } from '@utils/listViewOptions';

const STORAGE_KEY = 'list-view-options:permission';
...
const Permission = () => {
    ...
    const handleDelete = () => {
        showConfirm(
            t('pages.permission.deleteConfirmTitle'),
            t('pages.permission.deleteConfirmMessage', { count: selectedRows.size }),
            doDelete,
            closeModal
        );
    };
    ...
    const handleExportExcel = async () => {
        try {
            ...
            const response = await permissionExport({ keyword });

            const blob = new Blob([response.data], {
                type: response.headers['content-type'],
            });

            saveAs(blob, `permission_${new Date().toISOString().split('T')[0]}.xlsx`);
    ...
    return (
        ...
        >
            ...
            <Toolbar
                ...
                handleAdd={() =>
                    openModal(
                        t('pages.permission.addPermission'),
                        <RoleAdd onSubmit={addPermission} onClose={closeModal} />
                    )
                }
