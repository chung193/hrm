...
import { getAll, storage, bulkDestroy, categoryExport, update } from './CategoryServices.js';
import { DataGrid } from '@mui/x-data-grid';
import Toolbar from '@components/ToolBar';
import getColumns from './CategoryColumns.jsx';
import { getBreadcrumbs } from './CategoryBreadcrumb.js';
import CategoryAdd from './CategoryAddModal.jsx';
import { saveAs } from 'file-saver';
import MetaData from '@components/MetaData';
import { loadListViewOptions, saveListViewOptions } from '@utils/listViewOptions';

const STORAGE_KEY = 'list-view-options:category';
...
const Category = () => {
    ...
    const processRowUpdate = (newRow, oldRow) => {
        const payload = {
            ...
            parent_id: newRow.parent?.id ?? newRow.parent_id ?? null,
            sort_order: newRow.order ?? newRow.sort_order ?? 0,
        };
    ...
    const handleDelete = () => {
        showConfirm(
            t('pages.category.deleteConfirmTitle'),
            t('pages.category.deleteConfirmMessage', { count: selectedRows.size }),
            doDelete,
            closeModal
        );
    };
    ...
    const handleExportExcel = async () => {
        try {
            ...
            const response = await categoryExport({ keyword });

            const blob = new Blob([response.data], {
                type: response.headers['content-type'],
            });

            saveAs(blob, `category_${new Date().toISOString().split('T')[0]}.xlsx`);
    ...
    return (
        <MainCard
            pageTitle={t('pages.category.title')}
            pageDescription={t('pages.category.description')}
            breadcrumbs={breadcrumbs}
        >
            <MetaData title="Category Management" description="Category management page" />
            <Toolbar
                loadData={loadData}
                handleAdd={() =>
                    openModal(
                        t('pages.category.addCategory'),
                        <CategoryAdd onSubmit={addCategory} onClose={closeModal} />
                    )
                }
