...
import { getAll, storage, bulkDestroy, roleExport, update } from './RoleServices';
import { DataGrid } from '@mui/x-data-grid';
import Toolbar from '@components/ToolBar';
import getColumns from './RoleColumns.jsx';
import { getBreadcrumbs } from './RoleBreadcrumb';
import RoleAdd from './RoleAddModal.jsx';
import { saveAs } from 'file-saver';
import MetaData from '@components/MetaData';
import { loadListViewOptions, saveListViewOptions } from '@utils/listViewOptions';

const STORAGE_KEY = 'list-view-options:role';
...
const Role = () => {
    ...
    const handleDelete = () => {
        showConfirm(
            t('pages.role.deleteConfirmTitle'),
            t('pages.role.deleteConfirmMessage', { count: selectedRows.size }),
            doDelete,
            closeModal
        );
    };
    ...
    const handleExportExcel = async () => {
        try {
            ...
            const response = await roleExport({ keyword });

            const blob = new Blob([response.data], {
                type: response.headers['content-type'],
            });

            saveAs(blob, `role_${new Date().toISOString().split('T')[0]}.xlsx`);
            ...
            <Toolbar
                ...
                handleAdd={() =>
                    openModal(
                        ...
                        <RoleAdd onSubmit={addRole} onClose={closeModal} />
                    )
                }
