...
export default function AssetManagement() {
    ...
    const columns = useMemo(() => ({
        ...
        assignments: [
            ...
            { field: 'status', headerName: 'Status', width: 130, renderCell: (params) => <Chip size='small' label={params.value || '-'} color={getAssignmentChipColor(params.value)} variant='outlined' /> },
            {
                ...
                width: 170,
                sortable: false,
                renderCell: (params) => params.row.recall_requested_at
                    ? <Chip size='small' color='warning' variant='outlined' label={`Requested ${toDate(params.row.recall_requested_at)}`} />
                    : <Chip size='small' variant='outlined' label='Not requested' />,
            },
            {
                field: 'actions',
                headerName: 'Actions',
                width: 220,
                sortable: false,
                renderCell: (params) => params.row.status === 'assigned' ? (
                    <Stack direction='row' spacing={0.5}>
                        {!params.row.recall_requested_at && (
                            <Button size='small' color='warning' onClick={() => handleRecall(params.row.id)}>
                                Recall
                            </Button>
                        )}
                        <Button size='small' onClick={() => handleReturn(params.row.id)}>Return</Button>
                    </Stack>
                ) : null,
            },
        ],
    ...
    const handleRecall = async (id) => {
        showLoading();
        try {
            await requestRecallAssetAssignment(id, {});
            showNotification('Recall request sent successfully', 'success');
            await Promise.all([loadTab('assignments'), loadTab('assets')]);
        } catch (err) {
            showNotification(err.response?.data?.message || 'Recall request failed', 'error');
        } finally {
            hideLoading();
        }
    };
